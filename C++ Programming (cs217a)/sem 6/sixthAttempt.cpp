...
void itemSort(vector<string> item_Name, vector<int> item_Num, vector<int> p_Order, vector<double> man_Price,
...
readFile(inData, v_itemID, v_itemName, v_itemsCurrent, v_mfgPrice, v_storePrice, rows);  

int choice = 0; //choice of selection of what to do
     ...
     vector<double> sell_Price, int rows)
{
     ...
     for(firstOutOfOrder = 1; firstOutOfOrder < rows; firstOutOfOrder++)
        ...
        {
            temp1 = item_Name[firstOutOfOrder];
            ...
            do
            {
				item_Name[location] = item_Name[location - 1];
            ...
            while(location > 0 && itemName[location - 1] > temp2);
            
            item_Name[location] = temp1;
              ...
              double manuf_Price, double selling_Price, int rows)
{
         ...
         {
                //  itemID.push_back;
                  //inData >> item_ID.push_back(position);
                  //getline(item_Name.push_back(inData));
