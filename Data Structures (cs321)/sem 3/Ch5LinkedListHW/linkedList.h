...
Author: Trey Causey
Date: 5/29/08
Purpose: This file satifies all requirements of program 
		exercise 2, 3, 4, 5, and all a and b components of each.
**************************************************/
...
public:
	...
	void splitAt(linkedListType<Type> &secondList, 
				const Type& item);
		//Splits the list at the node with the info item into
		//two sublists.
		//Precondition: The list must exist.
		//Postcondition: first and last point to the first
		//	and last of the first sublist.  secondlist.first and
		//	secondlist.last point to the first and last nodes
...
template<class Type>
void linkedListType<Type>::splitAt(linkedListType<Type> &secondList, const Type &item)
{
	nodeType<Type> *current; //pointer to traverse the list
	nodeType<Type> *currentSub; //pointer to traverse the sublist
	nodeType<Type> *trailCurrentSub; //pointer to trail currentSub
    ...
    while(current != NULL && !found)		//search the list
	{
        if(current->info == item)     //the item is found
           found = true;
        else
		{        
			current = current->link; //make current point 
