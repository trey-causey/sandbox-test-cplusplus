...
public:
   ...
   bool search(const Type& searchItem);
	...
	Type getNode(int nodeNum);
		//Function to get the nth node in a list,
		//				where nth is nodeNum.
		//Postcondition:  Return the nodeNumth node.
	...
	void deleteSmallestNode();
		//Function to delete the first occurence of the smallest
		//				node in the list, traversing only one time.
		//Postcondition:  Node with smallest info value is delete
		//				on the first occurence.
	
	void deleteNodePos(int deleteItem);
		//Function to the nth node in the list, where n is deleteItem.
		//Postcondition:  The node at position deleteItem is deleted.

	void deleteAll(const Type& deleteItem);
		//Function to delete all occurences of deleteItem from the list.
	...
	void splitMid(linkedListType<Type> &sublist);
		...
		//  points to the the last node of the first sublist.
...
template<class Type>
Type linkedListType<Type>::getNode(int nodeNum)
{
	...
	else if(nodeNum > count)
		cerr << "There are not " << nodeNum << " numbers in this list.";
	
	current = first;

	while((x < count - 1) && (x < nodeNum - 1))
	{
...
void linkedListType<Type>::splitMid(linkedListType<Type> &sublist)
{
	...
	nodeType<Type> *sublistCurrent;
	int midPoint;
	int x;

	if(first == NULL)
		cerr << "Cannot copy from an empty list.";
	...
	for(x = 0; x < midPoint - 1; x++)
		current = current->link;

	tempCurrent = current->link;
	last = current;
	last->link = NULL;

	for(x = 0; x < count - midPoint - 1; x++)
	{
		if(x == 0)
		{
			current = tempCurrent;
...
template<class Type>
void linkedListType<Type>::deleteSmallestNode()
{
	nodeType<Type> *current; //pointer to traverse the list
	nodeType<Type> *smallestNode; //pointer to keep track of smallest node
	Type smallest; //Variable to keep track of smallest info

	if(first == NULL)    //Case 1; list is empty. 
		cerr<<"Can not delete from an empty list.\n";

	smallestNode = first;
	current = smallestNode->link;

	smallest = smallestNode->info;

	while(current != NULL)
	{
		if(smallestNode->info < current->info)
		{
			smallest = smallestNode->info;
			current = current->link;
		}//end if
...
template<class Type>
void linkedListType<Type>::deleteNodePos(int nodeNum)
{
	nodeType<Type> *current;
