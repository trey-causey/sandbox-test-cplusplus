...
public:
   ...
   void insertEnd(const elemType& insertItem);
    ...
    void removeAll(const elemType& removeItem);
        //Function to remove all of a particular item from the list.
        //The parameter removeItem specifies the item to be removed.
        //Postcondition: If removeItem is found in the list, it will be 
        //searched again until all instances are removed amd the list will
...
void arrayListType<elemType>::removeAt(int location)
{
	if(location < 0 || location >= length)
    	cerr<<"The location of the item to be removed "
			<<"is out of range."<<endl;
	else if(location <= (3/10 * length))
	{
         elemType temp = list[length - 1];
...
void arrayListType<elemType>::removeAll(const elemType& removeItem)
{
	...
	else
	{
		while(allRemoved == false)
		{
			...
			{
				cout<<"There are no more occurences in the list."
				<<endl;
				allRemoved = true;
			}
         }
